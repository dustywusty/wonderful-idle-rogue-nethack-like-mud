...
use crate::{
    common::NetProtocol,
    event::{CloseSocket, OpenSocket, SendSocket, SocketError, SocketOpened, SocketSent},
    listener::{Listener, ListenerTcp, ListenerUdp},
    socket::{Socket, SocketTcp, SocketUdp},
    sockets::Sockets,
    CloseListener, CreateListener, IpAddress, ListenerClosed, ListenerConnected, ListenerCreated,
    ListenerError, Listeners, NetError, SocketAddress, SocketClosed, SocketReceive,
};
...
pub fn open_socket_events_system(
    ...
    for open_socket_event in state.iter(&open_socket_events) {
        let sock = match open_socket_event.protocol {
            NetProtocol::Udp => Result::<Box<dyn Socket>, ()>::from(
                SocketUdp::connect(
                    open_socket_event.remote_address,
            ...
            NetProtocol::Tcp => Result::<Box<dyn Socket>, ()>::from(
                SocketTcp::connect(
                    open_socket_event.remote_address,
                    Some(open_socket_event.new_id),
                    open_socket_event.port,
...
pub fn send_socket_events_system(
    ...
    for send_socket_event in state.iter(&send_socket_events) {
        if let Ok(len) = sockets
            .get_mut(send_socket_event.id)
...
pub fn create_listener_events_system(
    ...
    for create_listener_event in state.iter(&create_listener_events) {
        let list = match create_listener_event.protocol {
            NetProtocol::Udp => Result::<Box<dyn Listener>, ()>::from(
                ListenerUdp::listen(
                    create_listener_event.port,
                    Some(create_listener_event.new_id),
                )
                .map(|l| Box::new(l) as Box<dyn Listener>),
            ),
            NetProtocol::Tcp => Result::<Box<dyn Listener>, ()>::from(
                ListenerTcp::listen(
                    create_listener_event.port,
                    Some(create_listener_event.new_id),
                )
...
pub fn close_listener_events_system(
    ...
    for close_listener_event in state.iter(&close_listener_events) {
        let listener = listeners
            .get_mut(close_listener_event.id)
